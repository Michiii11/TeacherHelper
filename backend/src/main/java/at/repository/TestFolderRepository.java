...
public class TestFolderRepository {
    ...
    public List<TestFolderDTO> getFolders(Long schoolId, String authToken) {
        ...
        return em.createQuery(
                        "SELECT f FROM TestFolder f WHERE f.school.id = :schoolId ORDER BY f.name ASC",
                        TestFolder.class
                )
    ...
    @Transactional
    public Response createFolder(Long schoolId, CreateTestFolderDTO dto) {
    ...
    @Transactional
    public Response updateFolder(String folderId, UpdateTestFolderDTO dto) {
        Long userId = tokenService.validateTokenAndGetUserId(dto.authToken());
        if (userId == null) {
            return Response.status(Response.Status.UNAUTHORIZED).build();
        }

        TestFolder folder = em.find(TestFolder.class, folderId);
        if (folder == null) {
    ...
    public Response deleteFolder(String folderId, String authToken) {
        ...
        Long childCount = em.createQuery(
                        "SELECT COUNT(f) FROM TestFolder f WHERE f.parent.id = :folderId",
                        Long.class
                )
    ...
    public TestFolder findById(String folderId) {
        return em.find(TestFolder.class, folderId);
    }

    public TestFolderDTO toDto(TestFolder folder) {
